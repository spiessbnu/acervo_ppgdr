...
def render_page_consultas(df: pd.DataFrame, embeddings: np.ndarray, matriz_similaridade: np.ndarray, subject_options: list):
    ...
    # >>> CORREÇÃO: seleção do AgGrid vem como list[dict], não DataFrame
    selected_rows = grid_response.get("selected_rows", [])
    if selected_rows:
        selected_idx = int(pd.DataFrame(selected_rows).iloc[0]["index_original"])
        st.session_state.selected_id = selected_idx
    else:
        st.session_state.selected_id = None
    ...
    with tab_detalhes:
        if st.session_state.selected_id is not None:
            detalhes = df.loc[st.session_state.selected_id]
            ...
            if isinstance(link_pdf, str) and link_pdf.strip():
    ...
    # Aba Trabalhos Similares
    with tab_similares:
        if matriz_similaridade is None or not matriz_similaridade.size:
            st.warning("Dados de similaridade não disponíveis.")
        elif st.session_state.selected_id is not None:
            id_selecionado = st.session_state.selected_id

            num_vizinhos = st.slider("Número de vizinhos", 1, 10, 5, 1)
            if (st.session_state.get('num_vizinhos_cache') != num_vizinhos):
...
def render_page_dashboard(df: pd.DataFrame, embeddings: np.ndarray):
    ...
    if todos_assuntos:
        ...
        fig_assuntos = px.bar(
            ...
            x='Quantidade', y='Assunto', orientation='h', title=' ',
            text='Quantidade'
        )
...
def main():
    ...
    # Carrega CSV (sempre necessário)
    df_raw = load_data(CSV_DATA_PATH)
    if df_raw is None:
        st.error("Falha ao carregar dados.")
        st.stop()

    # Padroniza nome da coluna e index original
    df = df_raw.rename(columns={"Tipo_Documento": "Tipo de Documento"})
    df['index_original'] = df.index

    # Páginas que exigem embeddings
    page_requires_embeddings = st.session_state.page in ("Consultas", "Dashboard")

    if page_requires_embeddings:
        embeddings = load_embeddings(EMBEDDINGS_PATH)
        if not validate_data(df, embeddings, require_embeddings=True):
            st.stop()
    else:
        embeddings = None
        if not validate_data(df, embeddings=None, require_embeddings=False):
            st.stop()

    # Similaridade completa apenas onde necessário
