...
def search_semantic(query_text: str, _document_embeddings: np.ndarray, model="text-embedding-3-large") -> list:
    ...
    try:
        ...
        return [i for i in np.argsort(-similarities) if similarities[i] > 0.25][:10]
    except Exception as e:
...
def get_ai_synthesis(summaries: str) -> str:
    ...
    try:
        client = openai.OpenAI(api_key=st.secrets["openai"]["api_key"])
        prompt_template = """Você é um especialista em análise de conteúdo e síntese acadêmica. Sua missão é analisar o conjunto de resumos de trabalhos acadêmicos fornecido. Leia todos os textos e identifique as conexões, os padrões e os temas centrais que os unem. Não resuma cada trabalho individualmente. Crie uma análise unificada que revele o panorama geral das pesquisas apresentadas. CONTEXTO (Resumos): --- {summaries} --- Sua resposta deve seguir rigorosamente o seguinte formato: **Síntese Analítica:** [Parágrafo denso e analítico que conecta as ideias principais dos textos.] **Temas Principais:** [Liste de 3 a 5 temas proeminentes em formato de lista com marcadores.]"""
